...
#define INNER_ARC_PUSH \
					...
					if ( !apparent_P)\
						...
						INNER_ARC_PUSH
							...
							if (i > 100) {
								VASER_DEBUG("trapped in loop: inc,ig_end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
								break;
							}
					}
				...
				else
				{
					...
					for (float a = angle1; ; a += dangle, i++)
					{
						...
						if (i > 100) {
							VASER_DEBUG("trapped in loop: inc,end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
							break;
						}
					}
				}
				...
				if (ignor_ends)
				{
					...
					for (float a = angle1 - dangle; a > angle2; a -= dangle, i++)
					{
						...
						INNER_ARC_PUSH
							...
							if (i > 100) {
								VASER_DEBUG("trapped in loop: dec,ig_end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
								break;
							}
					}
				}
...
#undef INNER_ARC_PUSH
						...
						if (i > 100) {
							VASER_DEBUG("trapped in loop: dec,end angle1=%.2f, angle2=%.2f, dangle=%.2f\n", angle1, angle2, dangle);
							break;
						}
					}
				}
...
#ifdef _DEBUG
		void annotate(const Point& P, Color cc, int I)
		{
			static int i = 0;
			if (I != -1) i = I;
...
#define push_quadf(A0,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,A11,A12) push_quadf_(__LINE__,A0,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10,A11,A12)
	...
	//see knife_cut_test for more info
		...
		{	//an iterative implementation
			...
			if (N > MAX_ST)
			{
				VASER_DEBUG("vah_N_knife_cut: max N for current build is %d\n", MAX_ST);
				N = MAX_ST;
			}
			...
			{	//2nd to 2nd last point
				...
				{ //must do intersection
					...
					if (result3) {
						...
						SL[i].vP = SL[i].T;
						SL[i].vR = SL[i].R;
						VASER_DEBUG("intersection failed: cos(angle)=%.4f, angle=%.4f(degree)\n", cos_tho, acos(cos_tho) * 180 / 3.14159);
					}
				}
