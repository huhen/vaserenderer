...
#include "includes/vertex_array_holder.h"
#include "includes/backend.h"

namespace VASEr
{
	vertex_array_holder::vertex_array_holder()
	{
		count = 0;
		glmode = GL_TRIANGLES;
		jumping = false;
	...
	void vertex_array_holder::set_gl_draw_mode(int gl_draw_mode)
	{
		glmode = gl_draw_mode;
	}

	void vertex_array_holder::clear()
	{
		count = 0;
	}

	void vertex_array_holder::move(int a, int b) //move b into a
	{
		vert[a * 2] = vert[b * 2];
	...
	void vertex_array_holder::replace(int a, Point P, Color C)
	{
		vert[a * 2] = P.x;
		vert[a * 2 + 1] = P.y;
	...
	/*int vertex_array_holder::draw_and_flush()
	{
		int& i = count;
		draw();
		switch (glmode)
	...
	int vertex_array_holder::push(const Point& P, const Color& cc, bool trans)
	{
		int cur = count;
		vert.push_back(P.x);
		vert.push_back(P.y);
	...
	void vertex_array_holder::push3(const Point& P1, const Point& P2, const Point& P3,
		const Color& C1, const Color& C2, const Color& C3,
		bool trans1, bool trans2, bool trans3)
	{
		push(P1, C1, trans1);
		push(P2, C2, trans2);
		push(P3, C3, trans3);
	...
	void vertex_array_holder::push(const vertex_array_holder& hold)
	{
		...
		{
			VASER_DEBUG("vertex_array_holder:push: unknown type\n");
		}
	}

	Point vertex_array_holder::get(int i)
	{
		Point P;
		P.x = vert[i * 2];
		P.y = vert[i * 2 + 1];
	...
	Color vertex_array_holder::get_color(int b)
	{
		Color C;
		C.r = color[b * 4];
		C.g = color[b * 4 + 1];
	...
	Point vertex_array_holder::get_relative_end(int di)
	...
	void vertex_array_holder::repeat_last_push()
	{
		Point P;
		Color cc;
	...
	void vertex_array_holder::jump() //to make a jump in triangle strip by degenerated triangles
	{
	...
	{
		VASErin::backend::vah_draw(*this);
	}

	void vertex_array_holder::draw_triangles()
	{
		Color col = { 1 , 0, 0, 0.5 };
	...
	void vertex_array_holder::swap(vertex_array_holder& B)
	{
